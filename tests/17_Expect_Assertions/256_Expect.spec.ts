...
test.describe('Expect Assertions - TestingAcademy', () => {
    ...
    test('2 Locator-based assertions', async ({ page }) => {
        ...
        const heading = page.getByText('multiple element filters', { exact: true }); // We should provide the exact value, here all small characters
        await expect(heading).toBeVisible();
        await expect(heading).toContainText('filter', { timeout: 10000 });

        const email = page.getByRole('textbox', { name: 'Email Address' });
        // assert with toHaveAttribute using id, type and placeholder
        ...
        await expect(footerLinks).toHaveCount(16); // count the footer links and assert
    ...
    test('3 soft assertions & Negation', async ({ page }) => {
        ...
        await expect(page.locator('#error')).not.toBeVisible(); // Negiation - Not visible
